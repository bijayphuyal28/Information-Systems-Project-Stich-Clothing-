(function( $ ) {
	...
	$.Shop.prototype = {
		init: function() {
			...
			//this.$shoppingCartActions = this.$element.find( "#shopping-cart-actions" ); // Cart actions links
			this.$btnPayment = this.$shoppingCartActions.find( "#btnpayment" ); // PayPal form
		...
		emptyCart: function() {
			...
			if( self.$btnPayment.length ) {
					self.$btnPayment.on( "click", function() {
					self._emptyCart();
					var url = "http://localhost:8090/Stichclothesshop/index.php";
